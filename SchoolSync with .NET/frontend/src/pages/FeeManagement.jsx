...
const FeeManagement = () => {
  ...
  // Razorpay Payment Handler (for students)
  const handleRazorpayPayment = async (fee) => {
    const scriptLoaded = await loadRazorpayScript();
    if (!scriptLoaded) {
      setMessage({ type: 'danger', text: 'Failed to load Razorpay. Please try again.' });
      return;
    }
    ...
    const options = {
      ...
      handler: async function (response) {
        // On successful payment, update backend
        try {
          ...
          setMessage({ type: 'danger', text: 'Payment succeeded but failed to update record.' });
        }
      },
      prefill: {
        name: user?.name || '',
  ...
  return (
    <>
      {/* Pay Fee Modal for Student (OUTSIDE main container) */}
              ...
              <table className="table table-hover">
                ...
                <tbody>
                  {fees.length > 0 ? (
                          ...
                          <td>
                            {fee.status === 'Pending' && (
                              <>
                                ...
                                >
                                  <i className="bi bi-credit-card"></i> Pay Online
                                </button>
                                {/* Optionally, keep the old modal for offline methods if needed */}
                                {/* <button className="btn btn-sm btn-success" onClick={() => openPayModal(fee.feeId)}>
                                  <i className="bi bi-cash"></i> Pay Offline
                                </button> */}
                              </>
                            )}
                          </td>
                    ...
                    <tr>
                      <td colSpan={user?.role === 'Admin' ? 8 : 7} className="text-center text-muted">No fee records found</td>
                    </tr>
                  )}
                </tbody>
              </table>
